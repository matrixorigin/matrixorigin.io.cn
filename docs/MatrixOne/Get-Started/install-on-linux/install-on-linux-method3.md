...
=== "稳定版本的镜像（3.0.10）"
      ...
      ```bash
      docker pull matrixorigin/matrixone:3.0.10
      docker run -d -p 6001:6001 --name matrixone matrixorigin/matrixone:3.0.10
      ```

      如果你使用的是中国大陆的网络，你可以拉取阿里云上的 MatrixOne 稳定版本镜像：

      ```bash
      docker pull registry.cn-shanghai.aliyuncs.com/matrixorigin/matrixone:3.0.10
      docker run -d -p 6001:6001 --name matrixone registry.cn-shanghai.aliyuncs.com/matrixorigin/matrixone:3.0.10
      ```
...
```bash
docker run -d -p 6001:6001 --name matrixone --privileged=true matrixorigin/matrixone:3.0.10
```
...
mo_ctl set_conf MO_CONTAINER_IMAGE="matrixorigin/matrixone:3.0.10" # 设置镜像，国内的可以用registry.cn-shanghai.aliyuncs.com/matrixorigin/matrixone:3.0.10 
