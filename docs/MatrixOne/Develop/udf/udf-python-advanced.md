...
本篇文档将指导你如何使用UDF进阶功能，包括以phython文件、whl包构建UDF。
...
2. 创建 UDF 函数
    ...
    '/opt/add_func.py' -- python文件在操作系统中的绝对路径
    handler 'add';
    ```
...
1. 创建 UDF 函数
    ...
    '/opt/udf/udf-1.0.0-py3-none-any.whl'   -- wheel包所在目录
    handler 'add_udf.add';   -- 指定调用whl包中add_udf模块的add函数
    ```
