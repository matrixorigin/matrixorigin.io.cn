...
|  数据类型   | 存储空间  |  精度  | 最大值 | 最小值| 语法表示 |
|  ----  | ----  |  ----  | ----  |
| FLOAT32  | 4 bytes | 	23 bits  |3.40282e+038（1038）|1.17549e-038（10-38）| FLOAT(M, D)<br> M 的取值范围为（1=< M <=255）。<br> D 的取值范围为（1=< D <=30），且 M >= D。|
| FLOAT64  | 8 bytes |  53 bits  |1.79769e+308（10308）|2.22507e-308（10-308）| DOUBLE(M, D) <br> M 的取值范围为（1=< M <=255）。<br> D 的取值范围为（1=< D <=30），且 M >= D。|

### **示例**

```sql
//Create a table named "float" with precision
