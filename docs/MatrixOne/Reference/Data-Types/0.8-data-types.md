...
|  数据类型   | 存储空间  |  精度  | 语法表示 |
|  ----  | ----  |  ----  | ----  |
| FLOAT32  | 4 bytes | 	23 bits  | FLOAT(M, D)<br> M 的取值范围为（1=< M <=255）。<br> D 的取值范围为（1=< D <=30），且 M >= D。|
| FLOAT64  | 8 bytes |  53 bits  | DOUBLE(M, D) <br> M 的取值范围为（1=< M <=255）。<br> D 的取值范围为（1=< D <=30），且 M >= D。|
