...
llms_summary: RANK() 为数据集中的每一行提供一个唯一的排名。RANK() 首先根据 ORDER BY 子句中指定的列对数据集进行排序，然后为每一行赋予一个唯一的排名。
---

# **RANK()**


> RANK() 为数据集中的每一行提供一个唯一的排名。RANK() 首先根据 ORDER BY
> 子句中指定的列对数据集进行排序，然后为每一行赋予一个唯一的排名。
