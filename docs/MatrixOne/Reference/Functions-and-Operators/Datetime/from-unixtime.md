...
llms_summary: '`FROM_UNIXTIME() 函数把内部 UNIX 时间戳值转换为普通格式的日期时间值，以 *YYYY-MM-DD HH:MM:SS* 或 *YYYYMMDDHHMMSS* 格式来显示。与 UNIX_TIMESTAMP ()` 函数互为反函数。'
---

# **FROM_UNIXTIME()**


> `FROM_UNIXTIME() 函数把内部 UNIX 时间戳值转换为普通格式的日期时间值，以 *YYYY-MM-DD HH:MM:SS*
> 或 *YYYYMMDDHHMMSS* 格式来显示。与 UNIX_TIMESTAMP ()` 函数互为反函数。
