...
llms_summary: JQ() 函数用于根据 jq 表达式解析和转换 JSON 数据。与 TRY_JQ() 不同的是，TRY_JQ() 支持出错时返回空值，而 JQ() 遇到错误则直接抛出异常。
---

# **JQ()**


> JQ() 函数用于根据 jq 表达式解析和转换 JSON 数据。与 TRY_JQ() 不同的是，TRY_JQ() 支持出错时返回空值，而
> JQ() 遇到错误则直接抛出异常。
